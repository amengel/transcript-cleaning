...
import csv
import os
import string # to test for punctuation
		...
		for d in docs:
			...
			for n in xrange(0,len(lines)):
				...
				if len(lines[n]) > 6:
					...
					elif lines[n][0:2] == '\xc2\xa9' and 'rights reserved.' in lines[n].strip().lower():
						copyright_position = n
					elif lines[n].strip()[0:3] == '(c)' and 'rights reserved.' in lines[n].strip().lower():
						copyright_position = n 
					elif lines[n][0:9] == 'Copyright' and 'rights reserved.' in lines[n].strip().lower():
						copyright_position = n 
					elif lines[n][0:8] == 'Document' and len(lines[n].strip().split(' ')) == 2:
						show_id = lines[n].strip().split(' ')[1]
						document_end_position = n
			
			if 'fox' in lines[word_count_position + 2].lower():
				show = lines[word_count_position + 2]
			else:
				show = lines[word_count_position + 3]
				...
				for c in ind:
					...
					for i in xrange(0,c+1):
						...
						# cleaning up for error handling given interjections
							...
							break
						elif txt[c-i] in string.whitespace and txt[c-i-1] in string.punctuation.replace(',', '').replace('"', '').replace('.', '').replace('&', '') and txt[c-i-2].isupper() == False:
							break
